 ...
 */
@Stateless
@Path("com.compguide.web.persistence.entities.procedure")
public class ProcedureFacadeREST extends AbstractFacade<Procedure> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Integer id, Procedure entity) {
        super.edit(entity);
    }
    ...
    public Procedure find(@PathParam("id") Integer id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Procedure> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
