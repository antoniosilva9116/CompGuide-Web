...
import com.compguide.web.Adapters.MedicationTaskAdapter;
...
import com.compguide.web.Persistence.SessionBeans.InteractionpairFacade;
...
import com.compguide.web.RxNormAPI.RxNormAPI;
...
public class ScheduleTaskController implements Serializable {
    ...
    @Inject
    private MedicationTaskAdapter medicationTaskAdapter;
    @EJB
    private com.compguide.web.Persistence.SessionBeans.InteractionpairFacade ejbInteractionpairFacade;
    @EJB
    ...
    @EJB
    private com.compguide.web.Persistence.SessionBeans.SimilarmedicationFacade ejbSimilarmedicationFacade;
        ...
        for (GuideExec guideExec : guideExecs) {
            ...
            if (list.isEmpty()) {
                ...
                for (ScheduleTask scheduleTask : list) {
                    if (scheduleTask.getCompleted() == false) {
                ...
                if (eventsNotificationLocal.isEmpty()) { // nao existem eventos e preciso criar
                    ...
                    for (DefaultScheduleEvent event : eventsSchedule) {
                        ...
                        if (dateNow.after(event.getEndDate()) == true) { // ultrapassou a data para ser verificada
                            ...
                            notificationsClone.add(notification); // adiciona a lista em memoria 
                        } else {
    ...
    public InteractionpairFacade getInteractionpairFacade() {
        return ejbInteractionpairFacade;
    }
    ...
    public SimilarmedicationFacade getSimilarmedicationFacade() {
        return ejbSimilarmedicationFacade;
    }

    public List<MedicationTask> getMedicationTasks() {
        if (!Objects.isNull(selected)) {
            medicationTasks = getMedicationTaskFacade().findByScheduleTask(selected);
        } else {
    ...
    public void setMedicationTasks(List<MedicationTask> medicationTasks) {
...
//  Notificar o utilizador quando se alcanÃ§ar os tempos mÃ­nimo e mÃ¡ximo de duraÃ§Ã£o 
    ...
    private List<DefaultScheduleEvent> processGuidelinePeriodicityHasDuration(Date fromStartDate,
            Date toEndDate,
        ...
        if (periodicity.isDuration() // o evento periodico da tarefa ocorre durante um determindo tempo 
            ...
            if (!periodicity.isCyclePart()) { // duracao sem parte de cada iteracao
                ...
                for (int i = 0; i < iterations; i++) {
                    ...
                    eventList.addAll(
                            processGuidelinePeriodicityHasDuration(
                                    newStartDate,
                                    endDate,
                                    periodicity,
                                    task,
...
//                getExternalContext().getSessionMap().get("processedTask");
    ...
    public void setQtd(boolean qtd) {
        this.qtd = qtd;
    }
    ...
    private void findSimilarMedication(List<ScheduleTask> scheduleTasks) {

        List<SimilarMedication> similarmedications = new ArrayList<>();

        for (ScheduleTask scheduleTask : scheduleTasks) {
            List<MedicationTask> medicationtasks = new ArrayList<>(getMedicationTaskFacade().findByScheduleTask(scheduleTask));

            if (Objects.isNull(medicationtasks)) {
                medicationtasks = new ArrayList<>();
            }

            for (MedicationTask medicationtask : medicationtasks) {
                List<ActiveIngredient> activeIngredients = medicationtask.getActiveIngredientList();

                if (Objects.isNull(activeIngredients)) {
                    activeIngredients = new ArrayList<>();
                }

                for (ActiveIngredient activeIngredient : activeIngredients) {
                    List<InteractionPair> interactionpairSTList = getInteractionpairFacade()
                            .findByActiveIngredientST(activeIngredient);

                    List<InteractionPair> interactionpairNDList = getInteractionpairFacade()
                            .findByActiveIngredientND(activeIngredient);

                    if (!interactionpairSTList.isEmpty() || !interactionpairNDList.isEmpty()) {
                        if (!Objects.isNull(activeIngredient.getSimilarMedicationList())
                                && activeIngredient.getSimilarMedicationList().isEmpty()) {
                            similarmedications.addAll(
                                    getSimilarMedication(activeIngredient)
                            );
                        }
                    }
                }
    ...
    private void findRecommendationInteractions(List<ScheduleTask> scheduleTasksST) {
        ...
        for (ScheduleTask scheduleTaskST : scheduleTasksST) {
            List<MedicationTask> medicationtasksST = new ArrayList<>(getMedicationTaskFacade().findByScheduleTask(scheduleTaskST));

            for (ScheduleTask scheduleTaskND : scheduleTasksND) {
                if (!Objects.equals(scheduleTaskST, scheduleTaskND)) {
                    for (MedicationTask medicationtaskST : medicationtasksST) {
                        List<MedicationTask> medicationNDList = new ArrayList<>(getMedicationTaskFacade()
                                .findByScheduleTask(scheduleTaskND));

                        for (MedicationTask medicationtaskND : medicationNDList) {

                            List<ActiveIngredient> activeIngredientsST = medicationtaskST.getActiveIngredientList();
                            List<ActiveIngredient> activeIngredientsND = medicationtaskND.getActiveIngredientList();

                            if (Objects.isNull(activeIngredientsST)) {
                                activeIngredientsST = new ArrayList<>();
                            }
                            if (Objects.isNull(activeIngredientsND)) {
                                activeIngredientsND = new ArrayList<>();
                            }

                            for (ActiveIngredient activeIngredientST : activeIngredientsST) {
                                for (ActiveIngredient activeIngredientND : activeIngredientsND) {

                                    if (!Objects.equals(activeIngredientST, activeIngredientND)) {
                                        InteractionPair interactionpair = getInteractionpairFacade()
                                                .findByActiveIngredientSTANDActiveIngredientND(
                                                        activeIngredientST,
                                                        activeIngredientND
                                                );

                                        if (interactionpair == null) {
                                            interactionpair = new InteractionPair();

                                            List<InteractionPair> interactionpairs = getMedicationInteraction(medicationtaskST, medicationtaskND);

                                            for (InteractionPair interactionpair1 : interactionpairs) {

                                                interactionpair = getInteractionpairFacade()
                                                        .findByActiveIngredientSTANDActiveIngredientND(
                                                                interactionpair1.getActiveIngredientSTID(),
                                                                interactionpair1.getActiveIngredientNDID()
                                                        );

                                                if (interactionpair == null) {
                                                    getInteractionpairFacade().create(interactionpair1);
                                                }
    ...
    private List<InteractionPair> getMedicationInteraction(MedicationTask medicationtaskST, MedicationTask medicationtaskND) {

        List<String> rxcuis = new ArrayList<>();
        List<MedicationTask> medicationtasks = Arrays.asList(medicationtaskST, medicationtaskND);

        for (ActiveIngredient activeIngredient : medicationtaskST.getActiveIngredientList()) {
            boolean edit = false;
            if (activeIngredient.getRxCui() == null) {
                String rxcuiST = RxNormAPI.instance().getRxcuiByName(activeIngredient.getName());
                activeIngredient.setRxCui(rxcuiST);
                edit = true;
            }

            if (activeIngredient.getClassID() == null) {
                String classIDST = RxNormAPI.instance().getClassIDByClassName(activeIngredient.getName());
                activeIngredient.setClassID(classIDST);
                edit = true;
            }

            if (edit == true) {
                getActiveIngredientFacade().edit(activeIngredient);
            }

            rxcuis.add(activeIngredient.getRxCui());
        }

        List<InteractionPair> interactionpairs = new ArrayList<>();

        for (ActiveIngredient activeIngredient : medicationtaskND.getActiveIngredientList()) {
            boolean edit = false;

            if (activeIngredient.getRxCui() == null) {
                String rxcuiND = RxNormAPI.instance().getRxcuiByName(activeIngredient.getName());
                activeIngredient.setRxCui(rxcuiND);
                edit = true;
            }

            if (activeIngredient.getClassID() == null) {
                String classIDND = RxNormAPI.instance().getClassIDByClassName(activeIngredient.getName());
                activeIngredient.setClassID(classIDND);
                edit = true;
            }
            ...
            for (String rxcui : rxcuis) {
                ...
                if (!Objects.equals(rxcui, activeIngredient.getRxCui())) {
                    List<InteractionPair> interactionpairList = RxNormAPI.instance().getInteraction(
                            Arrays.asList(rxcui, activeIngredient.getRxCui()),
                            medicationtaskST, medicationtaskND
                    );

                    interactionpairs.addAll(interactionpairList);
                }
            }
        }
    ...
    private List<SimilarMedication> getSimilarMedication(ActiveIngredient activeIngredient) {
        List<SimilarMedication> similarmedications = new ArrayList<>();

        if (activeIngredient.getClassID() != null) {
            List<SimilarMedication> similarmedicationList = RxNormAPI.instance().getSimilarByClassID(activeIngredient.getClassID(), activeIngredient);

            if (similarmedicationList.isEmpty()) {
                similarmedicationList = RxNormAPI.instance().getSimilarByRxcui(
                        activeIngredient.getRxCui(), activeIngredient
                );

            }

            for (SimilarMedication similarmedication : similarmedicationList) {

                SimilarMedication similarmedicationBD = getSimilarmedicationFacade()
                        .findByClassName(similarmedication.getClassName());

                if (similarmedicationBD == null) {
                    getSimilarmedicationFacade().create(similarmedication);
                    similarmedications.add(similarmedication);
                } else {
                    similarmedication.getActiveIngredientList().add(activeIngredient);
                    activeIngredient.getSimilarMedicationList().add(similarmedication);

                    getSimilarmedicationFacade().edit(similarmedication);
                    getActiveIngredientFacade().edit(activeIngredient);
                }
            }
